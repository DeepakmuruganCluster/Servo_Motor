...
const DEFAULT_STATE = {
  ...
  bs_preload_mu: 0.2,            // μ0 — preload nut friction coefficient
  bs_preload_force: 0,           // F0 — preload force (N)
...
function downloadExcelTemplate() {
  ...
  // Simple vertical layout: Row 1=title, Row 2=empty, Row 3=header, Rows 4+=Parameter|Value|Unit|Notes
  // col A=Parameter, col B=Value (editable), col C=Unit, col D=Notes (grey, read-only hint)
  const data = [
    ['Project Input Template', null, null, null],
    [],
    ['Parameter', 'Value', 'Unit', 'Notes'],
    ['Stn no',                                    'OP10',                                             '',    'Station identifier'],
    ['Appl',                                      'App-1',                                            '',    'Application name'],
    ['Stroke (mm)',                                step.stroke ?? 0,                                  '',    'Linear travel distance'],
    ['Move time (s)',                              step.move_time ?? 0,                               '',    'Total move time (accel + const + decel)'],
    ['Dwell time (s)',                             state.dwell_time ?? 0.1,                           '',    'Wait time at end of stroke'],
    ['Acceleration time (s)',                      step.acceleration_time ?? '',                       '',    'Accel phase duration (leave blank to use Acc% from app)'],
    ['Deceleration time (s)',                      step.deceleration_time ?? '',                      '',    'Decel phase duration (leave blank to use Dec% from app)'],
    ['External force (N)',                         step.external_force ?? 0,                          '',    'Process / payload force on the axis'],
    ['Ext. force direction',                       step.external_force_dir || 'opposing',             '',    'Options: opposing / aiding'],
    ['Payload movement direction',                 step.movement_dir || 'against gravity',            '',    'Options: against gravity / with gravity'],
    ['Payload mass (kg)',                          step.load_mass ?? 0,                              '',    'Mass of payload being moved'],
    ['Carriage mass (kg)',                         state.guide_mass ?? 0,                            '',    'LM guide carriage / saddle mass'],
    ['No. carriage blocks',                        state.guide_n_blocks ?? 1,                        '',    'Number of LM guide carriage blocks'],
    ['Inclination angle (°)',                      step.tilt_deg ?? 0,                               '',    '0 = horizontal, 90 = vertical'],
    ['Friction coeff LM guide (μ)',                state.mu ?? 0,                                    '',    'Friction coefficient of LM guide'],
    ['Friction Force (N)',                         state.guide_force ?? 0,                           '',    'Friction force per carriage block (N)'],
    ['Linear bush friction force (counterbalance)', state.cb_bushing_friction_force ?? 0,            '',    'Friction force per counterbalance bushing (N)'],
    ['number of linear bushings (counterbalance)', state.cb_n_bushings ?? 0,                        '',    'No. of counterbalance linear bushings'],
    ['Counterbalance exists',                      Number(state.has_counterbalance) ? 'yes' : 'no', '',    'Options: yes / no'],
    ['CB mass (kg)',                               state.cb_mass ?? 0,                               '',    'Counterbalance mass'],
    ['CB inclination (°)',                         state.cb_angle_deg ?? 90,                         '',    'Counterbalance inclination angle'],
    ['CB friction coeff (μ)',                      state.cb_mu ?? 0,                                 '',    'Counterbalance guide friction coefficient'],
    ['BS lead (mm)',                               state.bs_pitch ?? 2,                              '',    'Ball screw lead / pitch'],
    ['BS efficiency',                              state.bs_efficiency ?? 0.98,                      '',    'Ball screw mechanical efficiency (0–1)'],
    ['Preload nut friction coeff (μ0)',            state.bs_preload_mu ?? 0,                         '',    'Preload nut friction coefficient'],
    ['Preload force F0 (N)',                       state.bs_preload_force ?? 0,                      '',    'Ball screw preload force'],
    ['No. of fixed side support blocks',           state.bs_n_fixed_blocks ?? 1,                    '',    'No. of fixed-end bearing support blocks'],
    ['Fixed side drag torque per block (Nm)',      state.bs_fixed_drag_axial ?? 0,                   '',    'Drag torque per fixed-side block (from catalog)'],
    ['Fixed side support block drag torque (Nm)',  bearingDrag.fixed,                                '',    'Auto-calculated: No. blocks × drag/block'],
    ['No. of support side support blocks',         state.bs_n_support_blocks ?? 1,                  '',    'No. of floating-end bearing support blocks'],
    ['Support side drag torque per block (Nm)',    state.bs_support_drag_axial ?? 0,                 '',    'Drag torque per support-side block (from catalog)'],
    ['Support side support block drag torque (Nm)', bearingDrag.support,                            '',    'Auto-calculated: No. blocks × drag/block'],
    ['Bearing drag torque (Nm)',                   bearingDrag.total,                                '',    'Auto-calculated: fixed total + support total'],
    ['BS inertia (kg·m²)',                        state.bs_inertia ?? 0,                            '',    'Ball screw moment of inertia (from catalog)'],
    ['Guide mass (kg)',                            state.guide_mass ?? 0,                            '',    'LM guide carriage mass (from catalog)'],
    ['PK ratio',                                   state.pk_ratio ?? 1,                              '',    'Parallel kit gear ratio'],
    ['PK no-load (Nm)',                            state.pk_no_load_torque ?? 0,                     '',    'Parallel kit no-load running torque'],
    ['PK inertia (kg·m²)',                        state.pk_inertia ?? 0,                            '',    'Parallel kit moment of inertia'],
    ['PK max torque (Nm)',                         state.pk_max_torque ?? 0,                         '',    'Parallel kit max transferable torque'],
    ['PK max speed (rpm)',                         state.pk_max_speed ?? 0,                          '',    'Parallel kit max input speed'],
    ['GB ratio',                                   state.gb_ratio ?? 1,                              '',    'Gearbox ratio'],
    ['GB efficiency',                              state.gb_efficiency ?? 0,                         '',    'Gearbox mechanical efficiency (0–1)'],
    ['GB no-load (Nm)',                            state.gb_no_load_torque ?? 0,                     '',    'Gearbox no-load running torque'],
    ['GB inertia (kg·m²)',                        state.gb_inertia ?? 0,                            '',    'Gearbox reflected inertia'],
    ['Backlash (arcmin)',                          state.gb_backlash ?? 0,                           '',    'Gearbox backlash'],
    ['Safety factor (%)',                          state.safety_factor ?? 20,                        '',    'Design safety factor (%)'],
    ['Cycle time (s)',                             state.project_total_cycle ?? 7,                   '',    'Total machine cycle time'],
    ['Op. time (s)',                               state.project_operating_time ?? 2.6,              '',    'Servo operating time per cycle'],
    ['Shifts/day',                                 state.project_shifts ?? 3,                        '',    'Number of shifts per day'],
    ['Hours/shift',                                state.project_hours_shift ?? 7,                   '',    'Working hours per shift'],
    ['Days/week',                                  state.project_days_week ?? 6,                     '',    'Working days per week'],
    ['Service life (yrs)',                         state.project_service_life ?? 10,                 '',    'Required machine service life'],
    ['Accuracy (µm)',                              state.project_accuracy ?? 20,                     '',    'Required positioning accuracy'],
    ['Parallel Kit needed',                        state.pk_ratio > 1 ? 'Yes' : 'No',               '',    'Options: Yes / No'],
    ['Gearbox needed',                             state.gb_ratio > 1 ? 'Yes' : 'No',               '',    'Options: Yes / No'],
  ];

  const ws = XLSX.utils.aoa_to_sheet(data);
  ws['!cols'] = [{ wch: 46 }, { wch: 18 }, { wch: 8 }, { wch: 48 }];
  ...
  data.forEach((row, i) => {
    ...
    if (ws[`B${rowNum}`]) ws[`B${rowNum}`].s = isHelper ? helperStyle : inputStyle;
    if (ws[`D${rowNum}`]) ws[`D${rowNum}`].s = noteStyle;
  });

  // Excel formulas for bearing drag totals (yellow helper cells)
  ...
  if (rFixedT && rFixedN && rFixedB)   ws[`B${rFixedT}`]   = { t: 'n', f: `B${rFixedN}*B${rFixedB}`,   v: bearingDrag.fixed,   s: helperStyle };
  if (rSupportT && rSupportN && rSupportB) ws[`B${rSupportT}`] = { t: 'n', f: `B${rSupportN}*B${rSupportB}`, v: bearingDrag.support, s: helperStyle };
  if (rBearing && rFixedT && rSupportT) ws[`B${rBearing}`]  = { t: 'n', f: `B${rFixedT}+B${rSupportT}`, v: bearingDrag.total,   s: helperStyle };
...
function normalizeState() {
  ...
  state.bs_preload_mu          = Math.max(0, Number(state.bs_preload_mu)          || 0);
  state.bs_preload_force       = Math.max(0, Number(state.bs_preload_force)       || 0);
...
function calculateStepGroup(steps) {
  ...
  const results = {
    ...
    axial_force: 0,
    F_external: 0, F_gravity: 0, F_friction: 0, F_counterbalance: 0, F_counterbalance_friction: 0,
  ...
  steps.forEach(step => {
    ...
    const F_gravity_raw    = total_mass * G * (Math.sin(theta) + state.mu * Math.cos(theta));
    const gravity_sign     = step.movement_dir === 'with_gravity' ? -1 : 1;
    const F_gravity        = gravity_sign * F_gravity_raw;

    const ext_sign         = step.external_force_dir === 'aiding' ? -1 : 1;
    const F_external       = ext_sign * step.external_force;

    const F_friction       = state.guide_n_blocks * state.guide_force; // per-block × no. of carriage blocks
    ...
    const F_counterbalance_friction = (Number(state.cb_n_bushings) || 0) * (Number(state.cb_bushing_friction_force) || 0);

    const axial_force      = F_external + F_gravity + F_friction + F_counterbalance + F_counterbalance_friction;

    // --- Load Torque @ Ball Screw Input Shaft ---
    const T_axial_force  = pitch_m > 0 ? (Math.abs(axial_force) * pitch_m) / (2 * Math.PI * state.bs_efficiency) : 0;
    const T_preload      = (state.bs_preload_mu * state.bs_preload_force * pitch_m) / (2 * Math.PI);
    const bearingDrag    = getBearingDragComponents();
    // pk_no_load is at BS shaft — included in safety factor
    const T_bs_load      = T_axial_force + T_preload + bearingDrag.total + pkNoLoadTorque;

    const J_mass      = total_mass * Math.pow(pitch_m / (2 * Math.PI), 2);
    const J_reflected = J_mass + state.bs_inertia + pkInertia + gbInertia;
    const NscrewTerm   = Nscrew / 9.55;
    const T_accel      = J_reflected * (NscrewTerm * t_acc);
    const T_decel      = J_reflected * (NscrewTerm * t_dec);
    const T_peak_bs_acc = (T_bs_load + T_accel) * (1 + state.safety_factor / 100);
    const T_peak_bs_dec = (T_bs_load + T_decel) * (1 + state.safety_factor / 100);
    ...
    results.F_counterbalance_friction = Math.max(results.F_counterbalance_friction, F_counterbalance_friction);
    results.T_axial_force    = Math.max(results.T_axial_force,    T_axial_force);
...
function parseExcelValues(rows) {
  ...
  const rawMappings = [
    ...
    ['ballscrew preload nut friction',          'bs_preload_mu'],
    ['ballscrew preload force',                 'bs_preload_force'],
    ...
    ['preload nut friction coeffcient',         'bs_preload_mu'],
    ['preload nut friction coefficient',        'bs_preload_mu'],
    ['preload force',                           'bs_preload_force'],
...
function renderTorqueChart() {
  ...
  const segs = state.steps.map((step, i) => {
    ...
    // Axial force
    const theta = step.tilt_deg * Math.PI / 180;
    const F_grav_raw = total_mass * 9.81 * (Math.sin(theta) + state.mu * Math.cos(theta));
    ...
    const F_axial = ext_sign * step.external_force + grav_sign * F_grav_raw + state.guide_n_blocks * state.guide_force + F_cb + F_cb_friction;
    const T_axial = pitch_m > 0 ? Math.abs(F_axial) * pitch_m / (2 * Math.PI * state.bs_efficiency) : 0;
    const T_preload = (state.bs_preload_mu * state.bs_preload_force * pitch_m) / (2 * Math.PI);
    const bearingDragPreview = getBearingDragComponents();
    const T_bs_load = T_axial + T_preload + bearingDragPreview.total + pkNL;

    const J_ref  = (total_mass * Math.pow(pitch_m / (2 * Math.PI), 2)) + state.bs_inertia + (pkEnabled ? state.pk_inertia : 0) + (gbEnabled ? state.gb_inertia : 0);
    const NscrewTerm = (pitch_m > 0 ? (Vmax_m * 1000 / state.bs_pitch * 60) : 0) / 9.55;
    const T_acc  = J_ref * (NscrewTerm * t_acc);
    const T_dec  = J_ref * (NscrewTerm * t_dec);

    const T_load_m  = eff_ratio > 0 ? T_bs_load / (eff_ratio * eff_eff) + gbNL : 0;
    const T_peak_m  = eff_ratio > 0 ? Math.max(T_bs_load + T_acc, T_bs_load + T_dec) * (1 + state.safety_factor / 100) / (eff_ratio * eff_eff) + gbNL : 0;
    const T_decel_m = eff_ratio > 0 ? (T_bs_load + T_dec) * (1 + state.safety_factor / 100) / (eff_ratio * eff_eff) + gbNL : 0;
  ...
  let cursor = 0;
  segs.forEach(seg => {
    const x0 = tX(cursor), x1 = tX(cursor + seg.t_acc), x2 = tX(cursor + seg.t_acc + seg.t_const), x3 = tX(cursor + seg.t_acc + seg.t_const + seg.t_dec);
    const baseY = PAD.top + ph;

    // Accel fill
    ctx.fillStyle = '#3b82f618';
    ctx.fillRect(x0, tY(seg.T_peak_m), x1 - x0, baseY - tY(seg.T_peak_m));
    // Constant fill
    ctx.fillStyle = '#22c55e18';
    ctx.fillRect(x1, tY(seg.T_load_m), x2 - x1, baseY - tY(seg.T_load_m));
    // Decel fill
    ctx.fillStyle = '#f9731618';
    ctx.fillRect(x2, tY(Math.max(0, seg.T_decel_m)), x3 - x2, baseY - tY(Math.max(0, seg.T_decel_m)));

    // Outline
    ctx.beginPath();
    ctx.moveTo(x0, baseY);
    ctx.lineTo(x0, tY(seg.T_peak_m));
    ctx.lineTo(x1, tY(seg.T_peak_m));
    ctx.lineTo(x1, tY(seg.T_load_m));
    ctx.lineTo(x2, tY(seg.T_load_m));
    ctx.lineTo(x2, tY(Math.max(0, seg.T_decel_m)));
    ctx.lineTo(x3, tY(Math.max(0, seg.T_decel_m)));
    ctx.lineTo(x3, baseY);
    ctx.strokeStyle = seg.color; ctx.lineWidth = 2; ctx.stroke();

    // Peak label
    ctx.fillStyle = seg.color; ctx.font = 'bold 10px Inter,system-ui,sans-serif'; ctx.textAlign = 'center';
    ctx.fillText(`${seg.label} ${seg.T_peak_m.toFixed(3)}Nm`, tX(cursor + seg.t_acc / 2), tY(seg.T_peak_m) - 7);
    cursor += seg.t_acc + seg.t_const + seg.t_dec + seg.dwell;
  });
...
function downloadReport(result) {
  ...
  const primaryRows = [
    ...
    ['Acceleration torque at ball screw shaft', `${result.T_accel.toFixed(3)} Nm`],
    ['Deceleration torque at ball screw shaft', `${result.T_decel.toFixed(3)} Nm`],
