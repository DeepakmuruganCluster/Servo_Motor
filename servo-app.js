...
/* ══════════════════════════════════════════════
   ...
   One canvas per motor — trapezoidal velocity profile
   across all steps with dwell gaps between.
══════════════════════════════════════════════ */
function renderMotionProfileChart() {
  ...
  const PAD = { top: 24, right: 20, bottom: 48, left: 60 };
  const plotW = W - PAD.left - PAD.right;
  const plotH = H - PAD.top  - PAD.bottom;

  // Build segments for each step
  ...
  state.steps.forEach((step, i) => {
    ...
    const t_acc = Math.min(avail / 2, avail * (step.acc_pct / 100));
    const t_dec = t_acc;
    const t_const = Math.max(0, avail - t_acc - t_dec);
    const stroke_m = step.stroke / 1000;
    const Vmax = avail > t_acc ? stroke_m / (avail - t_acc) * 1000 : 0; // mm/s
    maxV = Math.max(maxV, Vmax);
    segments.push({ label: step.label || `Step ${i+1}`, t_acc, t_const, t_dec, dwell: state.dwell_time, Vmax, color: COLORS[i % COLORS.length] });
    totalTime += t_acc + t_const + t_dec + state.dwell_time;
  });
  ...
  for (let k = 0; k <= vSteps; k++) {
    const y = PAD.top + plotH - (k / vSteps) * plotH;
    const v = (k / vSteps) * maxV;
    ctx.beginPath();
  ...
  ctx.textAlign = 'center';
  ctx.fillStyle = '#6b7280';
  ctx.fillText('Time (s)', PAD.left + plotW / 2, H - 6);

  // Draw profiles
  let tCursor = 0;
  function tX(t) { return PAD.left + (t / totalTime) * plotW; }
  function vY(v) { return PAD.top + plotH - (v / maxV) * plotH; }

  segments.forEach(seg => {
    const pts = [
      [tCursor,                                      0],
      [tCursor + seg.t_acc,                          seg.Vmax],
      [tCursor + seg.t_acc + seg.t_const,            seg.Vmax],
      [tCursor + seg.t_acc + seg.t_const + seg.t_dec, 0],
      [tCursor + seg.t_acc + seg.t_const + seg.t_dec + seg.dwell, 0],
    ];

    // Fill under curve
    ctx.beginPath();
    ctx.moveTo(tX(pts[0][0]), vY(0));
    pts.forEach(([t, v]) => ctx.lineTo(tX(t), vY(v)));
    ctx.lineTo(tX(pts[pts.length-1][0]), vY(0));
    ctx.closePath();
    ...
    // Stroke profile
    ctx.beginPath();
    ctx.moveTo(tX(pts[0][0]), vY(pts[0][1]));
    pts.forEach(([t, v]) => ctx.lineTo(tX(t), vY(v)));
    ...
    // Step label at peak
    const midT = tCursor + seg.t_acc + seg.t_const / 2;
    ctx.fillStyle = seg.color;
    ctx.font = 'bold 11px Inter,system-ui,sans-serif';
    ctx.textAlign = 'center';
    ctx.fillText(seg.label, tX(midT), vY(seg.Vmax) - 6);

    tCursor += seg.t_acc + seg.t_const + seg.t_dec + seg.dwell;
  });
  ...
  ctx.textAlign = 'center';
  const tickCount = Math.min(10, Math.ceil(totalTime / 0.5));
  for (let k = 0; k <= tickCount; k++) {
    const t = (k / tickCount) * totalTime;
    const x = tX(t);
    ctx.beginPath();
  ...
  // Summary below chart
  const totalCycle = state.steps.reduce((s, step) => s + step.move_time, 0);
  if (legend) {
    legend.innerHTML = segments.map(seg =>
      `<span style="display:flex;align-items:center;gap:5px;">
        <span style="width:14px;height:14px;border-radius:3px;background:${seg.color};display:inline-block;"></span>
        ${seg.label} — ${(seg.t_acc + seg.t_const + seg.t_dec).toFixed(2)}s move + ${seg.dwell.toFixed(2)}s dwell
      </span>`
    ).join('') + `<span style="margin-left:auto;font-weight:600;color:var(--text)">Total cycle: ${totalCycle.toFixed(2)} s</span>`;
  }
}
...
function renderReport(result) {
  ...
  const overallPass = motor && Object.values(checks).every(v => v);
  const statusBadge = !motor
    ? `<span style="background:#fef3c7;color:#b45309;padding:2px 10px;border-radius:99px;font-size:12px;font-weight:700;">NO MOTOR</span>`
    : overallPass
      ? `<span style="background:#dcfce7;color:#15803d;padding:2px 10px;border-radius:99px;font-size:12px;font-weight:700;">PASS</span>`
      : `<span style="background:#fee2e2;color:#b91c1c;padding:2px 10px;border-radius:99px;font-size:12px;font-weight:700;">FAIL</span>`;

  const totalCycle = state.steps.reduce((s, st) => s + st.move_time, 0).toFixed(2);

  const row = (label, value) => `<tr><td style="padding:6px 10px;color:#6b7280;font-size:13px;">${label}</td><td style="padding:6px 10px;font-size:13px;font-weight:600;">${value}</td></tr>`;

  body.innerHTML = `
    <div style="display:flex;align-items:center;gap:12px;margin-bottom:20px;">
      <img src="clustervise-logo.png" alt="ClusterVise" style="height:32px;" />
      <div style="flex:1;"></div>
      <div style="text-align:right;font-size:12px;color:#6b7280;">Generated ${new Date().toLocaleDateString(undefined,{day:'numeric',month:'short',year:'numeric'})}</div>
      ${statusBadge}
    </div>

    <div style="display:grid;grid-template-columns:1fr 1fr;gap:24px;margin-bottom:24px;">
      <div>
        <div style="font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:.06em;color:#6b7280;margin-bottom:8px;">Mechanical Configuration</div>
        <table style="border-collapse:collapse;width:100%;">
          ${row('Ball screw pitch', state.bs_pitch + ' mm')}
          ${row('BS efficiency', state.bs_efficiency)}
          ${row('BS friction torque', state.bs_friction_torque + ' Nm')}
          ${row('PK ratio', state.pk_ratio + ':1')}
          ${row('PK no-load torque', state.pk_no_load_torque + ' Nm')}
          ${row('GB ratio', state.gb_ratio + ':1')}
          ${row('GB efficiency', state.gb_efficiency)}
          ${row('Safety factor', state.safety_factor + '%')}
        </table>
      </div>
      <div>
        <div style="font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:.06em;color:#6b7280;margin-bottom:8px;">Key Results</div>
        <table style="border-collapse:collapse;width:100%;">
          ${row('Peak motor torque', result.T_peak_motor.toFixed(3) + ' Nm')}
          ${row('RMS motor torque', result.T_rms_motor.toFixed(3) + ' Nm')}
          ${row('Motor speed', Math.round(result.Nmotor) + ' rpm')}
          ${row('Reflected inertia', (result.I_motor * 1e6).toFixed(3) + ' kg·mm²')}
          ${row('Inertia ratio', result.inertia_ratio !== null ? result.inertia_ratio.toFixed(2) : '—')}
          ${row('Total cycle time', totalCycle + ' s')}
          ${row('Motion steps', state.steps.length)}
        </table>
      </div>
    </div>

    <div style="font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:.06em;color:#6b7280;margin-bottom:8px;">Motion Steps</div>
    <table style="border-collapse:collapse;width:100%;margin-bottom:24px;">
      <thead><tr style="background:#f3f4f6;">
        <th style="padding:7px 10px;text-align:left;font-size:12px;">Step</th>
        <th style="padding:7px 10px;text-align:right;font-size:12px;">Stroke (mm)</th>
        <th style="padding:7px 10px;text-align:right;font-size:12px;">Move time (s)</th>
        <th style="padding:7px 10px;text-align:right;font-size:12px;">Accel %</th>
        <th style="padding:7px 10px;text-align:right;font-size:12px;">Ext. force (N)</th>
        <th style="padding:7px 10px;text-align:right;font-size:12px;">Mass (kg)</th>
        <th style="padding:7px 10px;text-align:right;font-size:12px;">Tilt (°)</th>
      </tr></thead>
      <tbody>
        ${state.steps.map((s, i) => `<tr style="border-top:1px solid #e5e7eb;">
          <td style="padding:6px 10px;font-size:13px;">${s.label || 'Step '+(i+1)}</td>
          <td style="padding:6px 10px;font-size:13px;text-align:right;">${s.stroke}</td>
          <td style="padding:6px 10px;font-size:13px;text-align:right;">${s.move_time}</td>
          <td style="padding:6px 10px;font-size:13px;text-align:right;">${s.acc_pct}</td>
          <td style="padding:6px 10px;font-size:13px;text-align:right;">${s.external_force}</td>
          <td style="padding:6px 10px;font-size:13px;text-align:right;">${s.load_mass}</td>
          <td style="padding:6px 10px;font-size:13px;text-align:right;">${s.tilt_deg}</td>
        </tr>`).join('')}
      </tbody>
    </table>

    <div style="font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:.06em;color:#6b7280;margin-bottom:8px;">Selected Motor</div>
    ${motor ? `<table style="border-collapse:collapse;width:100%;margin-bottom:8px;">
      ${row('Part number', motor.pn)}
      ${row('Series', motor.series)}
      ${row('Rated torque (M₀)', motor.M0.toFixed(2) + ' Nm')}
      ${row('Peak torque (Tmax)', motor.Tmax.toFixed(2) + ' Nm')}
      ${row('Max speed (Nmax)', motor.Nmax + ' rpm')}
      ${row('Rotor inertia (J)', motor.J + ' kg·cm²')}
    </table>` : `<p style="font-size:13px;color:#b45309;">No motor selected. Click a row in the Motor Catalog.</p>`}`;
